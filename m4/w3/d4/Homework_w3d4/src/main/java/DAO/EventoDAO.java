...
//		- getEventiPerInvitato(Persona invitato) **** DIFFICILE ****
	...
	public static void getConcertiInStreaming(boolean is_stream) {
		...
		Query q = em.createQuery("SELECT p FROM Evento p", Evento.class);

		List<Evento> res = q.getResultList();

		res.stream().filter(e -> e instanceof Concerto).map(e -> (Concerto) e)
				.filter(e -> e.isIn_streaming() == (is_stream)).collect(Collectors.toList());
	...
	public static void getConcertiPerGenere(GenereConcerto tipo_concerto) {
		...
		res.stream().filter(e -> e instanceof Concerto).map(e -> (Concerto) e)
				.filter(e -> e.getGenere_concerto().equals(tipo_concerto)).collect(Collectors.toList());
