...
public class Dipendente {
	...
	public int getStiTotaleStraordinari(int h) {
		int StiTotaleStraordinari = (int) (getStipTotale() + (importoStraordinario * h));
		//System.out.println(StiTotaleStraordinari);
		return StiTotaleStraordinari;
	}
	...
	public void stampaDipendente() {
		System.out.printf("Il dipendente con la matricola: %s che lavora come %s nel dipartimento %s e il suo stipendio è %d %n %n",
				matricola,
				livello.getRuolo(),
				sezione,
				getStipTotale()
	...
	public role promuoviDipendente() {
		...
		case QUADRO:
			...
			break;
		case DIRIGENTE:
			System.out.println("Non lo so già sei Dirigente che voi di più? c'è boh...");
			break;
		}
		
		System.out.println("Il dipendente " + matricola + " è adesso un " + livello.getRuolo());
